...
if (isset($_SERVER["REQUEST_METHOD"]) && $_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
        header("Location: index.php?message=Invalid email format.");
        exit;
    }
    ...
    if ($mail->send()) {
        ...
        header("Location: index.php?message=Email sent successfully.");
        exit;
    } else {
        // Redirect back with error message
        header("Location: index.php?message=Error sending email: " . $mail->ErrorInfo);
        exit;
    }
} else {
    // If form not submitted, redirect back to form
    header("Location: index.php?message=Method not allowed.");
    exit;
}
